    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        if ( !phoneNum.test(values.userName) ) {    //判断手机号码是否正确
          ...
          //   mobile: values.userName,
          //   password: values.password,
          //   ticket: this_.state.ticket,
          //   randstr: this_.state.randstr,
          // })
          // .then(function (response) {   //调用接口成功执行
          //   console.log(response);
          //   // 登陆成功前 将token保存到本地
      ...
      <div>
        ...
        <Form onSubmit={this.handleSubmit} className="login-form" style={{width: "90%"}}>
          ...
          <FormItem style={{ width: "90%", margin: "0 auto" }}>
            ...
            <Button type="primary" htmlType="submit" className="login-form-buttons" id="TencentCaptcha" data-appid="2020178866" data-cbfn="callback">立即登录</Button>
