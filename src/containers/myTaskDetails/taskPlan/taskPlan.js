...
class TaskPlan extends Component {
  ...
  render() {
    ...
    const { fav_flow_content,task_type,platformname,addition_pic,is_addcomments,receive_evaluate_content,taobao_ordersn, addtime, need_principal, order_id, user_taobao, itemprice, itemnum, chat_pay_content, order_status } = this.props;
    return(
      <div className="task-plan">
        {
          task_type === "1" ?
            <div>
              ...
              {/* 任务要求 */}
              <div className="plan-box">
                <p className="title"><b className="plan-title">一：订单详情</b><span></span></p>
                <div className="task-plan-list"><span>浏览收藏加购</span>
                  <div className="task-plan-list-img">
                    {/* 聊天下单截图反显 */}
                    {
                      fav_flow_content ?
                        fav_flow_content.map((item, index) => {
                          return(
              ...
              {/* 任务完成 */}
              <div className="plan-box">
                <p className="title"><b className="plan-title">二：完成任务</b><span></span></p>
                <div className="task-plan-list">
                  <span className="ask-start">平台规定商家48小时内返款</span>
                  <button style={ order_status === 4 ? {backgroundColor: '#66caa8'} : {backgroundColor: '#ccc'} }>获得佣金</button>
                </div>
              </div>
            </div>
          :
