...
class Correct_jingdongs extends Component {
  constructor(props) {
    super();
    this.state = {
      animating: false,
  ...
  componentDidMount () {
    ...
    if ( this.props.location.state === undefined ) {
      ...
      .then(function (response) {   //调用接口成功执行
        ...
        let datas = [responses.url1,responses.url2,responses.url3,responses.url4];        //存储后台放回过来的图片路径
        ...
        this_.setState({
          ...
          url4: responses.url4.url,                 //存储返回的图片
        })
      }
      })
  ...
  handleSubmit = (e) => {
    ...
    let fImgs = [states.url1, states.url2, states.url3, states.url4];     //储存后台放回需要反显得图片
...
const Correct_jingdong = Form.create()(Correct_jingdongs)
export default Correct_jingdong
